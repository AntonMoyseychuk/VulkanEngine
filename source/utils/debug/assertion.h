...
void AssertImpl(bool condition, int loggerType, const char* file, const char* function, uint32_t line, const char* conditionStr, const char* message);
    ...
    #define AM_ASSERT(condition, formatt, ...)              AssertImpl(condition, Logger::LoggerType_COMMON, __FILE__, __FUNCTION__, __LINE__, "condtion: " #condition, fmt::format(formatt, __VA_ARGS__).c_str())
